...
export async function POST(req:NextRequest) {
    try{
        ...
        const {user,Name:N} = await req.json();
        
        await ConnectToDb();
        const IsUser = await userModel.findOne({Email:user.email});
        
        if(!IsUser) return new Response(null,{status:404});
        
        const Name = N.replaceAll('%20'," ").replaceAll('%40',"").replaceAll('%80',"")
        const CuUser = await userModel.findOne({Name:Name});
